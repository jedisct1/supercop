...
/* construct a random 3-quasi LRPC (3 blocks of circulant) */
struct LRP gen_3Q(void)
{
	short i,j,t,info;
	short n,k,nk,nkd;
	short blk2,blk3;
...
int crypto_encrypt_keypair(unsigned char *pk, unsigned char *sk)
{
	...
	while( 2 )
	{
		...
		/* compute F = G' * H^T (2 blocks of circulant) */
		F = gen_matrix(l,nk);
		...
		/* check that F contains identity matrix */
	...
	for( i=0 ; i<blk ; i++ )
	{
		...
		gf_dat[blk*2+i] = F.dat[blk][i];
	}

	bin_dat = gf2bin(gf_dat,gf_size,&bin_size);
	pk_size = (_m * blk * 3 + 7)/8;
...
void encrypt_one_block(unsigned char *c, const unsigned char *m, const unsigned char *pk)
{
	short i,j;
	short n,k,l,nk;
	...
	for( j=0 ; j<nk ; j++ )
	  F.dat[nk][j] = gf_dat[blk*2+j];

	for( i=blk+1 ; i<l ; i++ )  /* circulant */
	{
		F.dat[i][0] = F.dat[i-1][blk-1];
	...
	/* generate a error vector with rank r */
...
int decrypt_one_block(unsigned char *m, const unsigned char *c, const unsigned char *sk)
{
	int result = 0;
	short i,j,t;
	short n,k,l,nk,nkd,bh;
	...
	/* recover parity check matrix (3 blocks of circulant) */
	H.M = gen_matrix(nk,n);
	H.basis = (gf *)malloc(d * sizeof(*H.basis));
	H.bin = (Bvec *)malloc(nkd * sizeof(*H.bin));

	for( i=0 ; i<d ; i++ )
	  H.basis[i] = gf_dat[blk+i];
	...
	for( i=0 ; i<nkd ; i+=blk )
	{
		for( j=1 ; j<blk ; j++ )
		{
			...
			put_elt(&H.bin[i+j],0,get_elt(H.bin[i+j-1],blk-1));
			put_elt(&H.bin[i+j],blk,get_elt(H.bin[i+j-1],blk*2-1));
			put_elt(&H.bin[i+j],blk*2,get_elt(H.bin[i+j-1],blk*3-1));
		}
	}
...
/* find the dual space of the subspace generated by given n vectors
...
/* find error space using the syndrome vector with length N and the basis of parity check matrix */
