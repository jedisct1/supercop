/*
 * crypto_auth/try.c version 20200406
 * D. J. Bernstein
 * Public domain.
...
void test(void)
{
  ...
  for (loop = 0;loop < LOOPS;++loop) {
    ...
    if (crypto_auth(h2,m2,mlen,k2) != 0) fail("crypto_auth returns nonzero");
    if (memcmp(h2,h,hlen) != 0) fail("crypto_auth is nondeterministic");
    
    double_canary(h2,h,hlen);
    double_canary(m2,m,mlen);
    double_canary(k2,k,klen);
    if (crypto_auth(m2,m2,mlen,k) != 0) fail("crypto_auth with m=h overlap returns nonzero");
    if (memcmp(m2,h,hlen) != 0) fail("crypto_auth does not handle m=h overlap");
    memcpy(m2,m,mlen);
    if (crypto_auth(k2,m,mlen,k2) != 0) fail("crypto_auth with k=h overlap returns nonzero");
    ...
    double_canary(m2,m,mlen);
    double_canary(k2,k,klen);
    if (crypto_auth_verify(h2,m2,mlen,k2) != 0) fail("crypto_auth_verify returns nonzero");
