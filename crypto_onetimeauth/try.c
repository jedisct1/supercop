/*
 * crypto_onetimeauth/try.c version 20240530
 * D. J. Bernstein
 * Public domain.
...
void test(void)
{
  ...
  for (loop = 0;loop < LOOPS;++loop) {
    ...
    poison(m,mlen);
    poison(k,klen);
    result = crypto_onetimeauth(h,m,mlen,k);
    ...
    poison(m2,mlen);
    poison(k2,klen);
    result = crypto_onetimeauth(h2,m2,mlen,k2);
    unpoison(&result,sizeof result);
    if (result != 0) fail("crypto_onetimeauth returns nonzero");
    unpoison(h2,hlen);
    unpoison(m2,mlen);
    ...
    poison(m2,mlen);
    poison(k,klen);
    result = crypto_onetimeauth(m2,m2,mlen,k);
    unpoison(&result,sizeof result);
    ...
    poison(m,mlen);
    poison(k2,klen);
    result = crypto_onetimeauth(k2,m,mlen,k2);
    unpoison(&result,sizeof result);
    ...
    poison(m,mlen);
    poison(k,klen);
    result = crypto_onetimeauth_verify(h,m,mlen,k);
    unpoison(&result,sizeof result);
    ...
    poison(m2,mlen);
    poison(k2,klen);
    result = crypto_onetimeauth_verify(h2,m2,mlen,k2);
    unpoison(&result,sizeof result);
    if (result != 0) fail("crypto_onetimeauth_verify returns nonzero");
    unpoison(h2,hlen);
    unpoison(m2,mlen);
