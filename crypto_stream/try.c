/*
 * crypto_stream/try.c version 20240530
 * D. J. Bernstein
 * Public domain.
...
void test(void)
{
  ...
  for (loop = 0;loop < LOOPS;++loop) {
    ...
    unpoison(n,nlen);
    poison(k,klen);
    result = crypto_stream(s,slen,n,k);
    ...
    unpoison(n2,nlen);
    poison(k2,klen);
    result = crypto_stream(s2,slen,n2,k2);
    unpoison(&result,sizeof result);
    if (result != 0) fail("crypto_stream returns nonzero");
    unpoison(s2,slen);
    unpoison(n2,nlen);
    ...
    unpoison(n,nlen);
    poison(k,klen);
    result = crypto_stream_xor(c,m,mlen,n,k);
    unpoison(&result,sizeof result);
    ...
    unpoison(n2,nlen);
    poison(k2,klen);
    result = crypto_stream_xor(c2,m2,mlen,n2,k2);
    unpoison(&result,sizeof result);
    if (result != 0) fail("crypto_stream_xor returns nonzero");
    unpoison(c2,clen);
    unpoison(m2,mlen);
    ...
    unpoison(n,nlen);
    poison(k,klen);
    result = crypto_stream_xor(m2,m2,mlen,n,k);
    unpoison(&result,sizeof result);
    ...
    unpoison(n2,nlen);
    poison(k,klen);
    result = crypto_stream_xor(n2,m,mlen,n2,k);
    unpoison(&result,sizeof result);
    ...
    unpoison(n,nlen);
    poison(k2,klen);
    result = crypto_stream_xor(k2,m,mlen,n,k2);
    unpoison(&result,sizeof result);
