...
#include "int64_minmax_x86.inc"

/* k is a multiple of 4p */
/* p is a multiple of 2q */
/* p is a power of 2 */
static inline void twolevels_down(int64 *x,int64 k,int64 p,int64 q)
{
  int64 i;
  x += k;
  while (p > 0) {
...
static inline void twolevels_up(int64 *x,int64 k,int64 p,int64 q)
{
  int64 i;
  x += k;
  while (p > 0) {
...
/* p is a power of 2 */
static inline void threelevels_down(int64 *x,int64 k,int64 p,int64 q)
{
  int64 i;
  x += k;
  while (p > 0) {
...
static inline void threelevels_up(int64 *x,int64 k,int64 p,int64 q)
{
  int64 i;
  x += k;
  while (p > 0) {
...
static inline void crypto_sort_smallindices(int64 *x,int64 n)
{ int64 t,p,q,i,j,k;
  int64 x0,x1,x2,x3,x4,x5,x6,x7;
  ...
  if (k+4 <= n) {
    int64_MINMAX(x[k+1],x[k]);
    int64_MINMAX(x[k+2],x[k+3]);
    if (k+6 < n)
      int64_MINMAX(x[k+5],x[k+4]);
    else if (k+6 <= n)
      int64_MINMAX(x[k+4],x[k+5]);
  } else {
    if (k+2 < n)
      int64_MINMAX(x[k+1],x[k]);
    else if (k+2 <= n)
      int64_MINMAX(x[k],x[k+1]);
  }
