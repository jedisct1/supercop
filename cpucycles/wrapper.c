...
// 20260105 djb: add orderbump
// 20260105 djb: catch abort() in works(); bump version
// 20251226 djb: add ticks_close()
// 20251226 djb: use sigjmp_buf instead of jmp_buf (this matters on solaris)
// 20250925 djb: loosen 0.1 to 0.2 for FINDMULTIPLIER
// 20230115 djb: cpucycles_version()
// 20230106 djb: support "cpu MHz static" (ibm z15)
// 20230105 djb: first version; includes some pieces adapted from supercop
...
int cpucycles_works(long long (*ticks)(void))
{
  ...
  sigaction(SIGABRT,0,&old_sigabrt);
  ...
  if (!sigsetjmp(crash_jmp,1)) {
    ...
    sigaction(SIGABRT,&crash_action,0);
    ticks();
    result = 1;
  }
  ...
  sigaction(SIGABRT,&old_sigabrt,0);
...
long long cpucycles_init(void)
{
  ...
  long long bestopt;
  long long opt;
  long long orderbump = 0;

  persecond = osfreq();

  for (opt = 0;opt < NUMOPTIONS;options[opt++].ticks_close()) {
    ...
    for (tries = 0;tries < 10;++tries) {
      ...
      if (ok) {
        ...
        if (freq == cpucycles_MAYBECYCLECOUNTER || freq == cpucycles_FINDMULTIPLIER)
          precision[opt] += 100;
        if (freq > 0 || freq == cpucycles_EXTEND32)
          precision[opt] += 300;

        precision[opt] += orderbump;
        orderbump += 10;
