...
/* AES-OTR v3 for combined associated data processing (aka OTRC) optimized for AESNI-capable CPU */
...
/*
EFunc : OTR Core Encryption Function, for ADP=C
*/
block EFunc(
	const block *V,
	...
	uint8 *ciphertext)
{
	...
	AES_encrypt(*V, &Ln[0], encrypt_key);
...
/*
DFunc : OTR Core Decryption Function, for ADP=C
*/
block DFunc(
	...
	uint8 *plaintext)
{
	...
	if (adproc == AUTHF){//Associated data processing 
		dd = 0;
		dd2 = 0;
		ptp = (block *)malloc(sizeof(block)*(2*PIPE));
	}
	AES_encrypt(*V, &Ln[0], encrypt_key);
	while (rest_len > (DBLOCK*PIPE)){
		/* first round*/
...
#endif
	if ((uint32)adlen > 0){
		V = DFunc(AUTHF, &cst, ad, (uint32) adlen, NULL);
	}
	V = _mm_xor_si128(V, fmt(npub));
	TE = EFunc(&V, m, (uint32)mlen, c);
	...
	)
{
	...
	*mlen = clen - CRYPTO_ABYTES;
	...
	V = _mm_xor_si128(V, fmt(npub));
	TE = DFunc(DECF, &V, c, (uint32)*mlen, m);
	if (memcmp(&TE, c + (uint32)*mlen, CRYPTO_ABYTES) != 0){//non-constant-time compare
		return TAG_UNMATCH;
	}
	return TAG_MATCH;
}
