...
void DeoxysEncrypt_Checksum(__m128i rk[15][8], const uint8_t* pt, uint8_t ct[128], __m128i* Checksum) {
    __m128i b[8],a[8];
    unsigned u;

    for(u=0;u<8;u++)  a[u]=LOAD(pt+16*u);
    XOR_8(a,*Checksum);
    packing(a);
    /* First AddRoundTweakey */
    ADDKEY(a,rk[0]);
...
void DeoxysDecrypt_Checksum(__m128i rk[15][8], const uint8_t* ct, uint8_t pt[128], __m128i* Checksum) {

    __m128i d[8];
    __m128i r[8];
    unsigned u;

    for(u=0;u<8;u++) d[u]=LOAD(ct+16*u);
    packing(d);
    /*1 to Nr rounds*/
    for(u=0;u<14;u++) {
   	    ADDKEY(d,rk[14-u]);
    ...
    unpacking(d);
    XOR_8(d,*Checksum);
    for(u=0;u<8;u++) STORE(pt+u*16,d[u]);
}
