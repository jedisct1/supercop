...
void acorn128_32steps(unsigned char *state, const unsigned char *inbyte, unsigned char *outbyte, unsigned char cabyte, unsigned char cbbyte, unsigned char enc_dec_flag)
{
    unsigned int i;
    unsigned char j,f;
	...
	)
{
    ...
    *mlen = clen - 16;
    ...
    for (i = 0; i < (*mlen & 0xfffffffffffffffcULL); i=i+4)
    {
        acorn128_32steps(state, &(c[i]), &m[i], ca, cb,0);
    }
