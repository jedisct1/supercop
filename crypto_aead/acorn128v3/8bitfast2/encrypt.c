/*
  ...
  In this program, we shift the register every 64 steps (64 bits)
...
void acorn128_64steps(unsigned char *state, unsigned char *plaintextbyte, unsigned char *ciphertextbyte, unsigned char cabyte, unsigned char cbbyte, unsigned char enc_dec_flag)
{
    ...
    for (j = 0;  j <= 36; j++) state[j] = state[j+8];
    for (j = 37; j <= 44; j++) state[j] = 0;
}
...
void acorn128_initialization(const unsigned char *key, const unsigned char *iv, unsigned char *state)
{
    ...
    unsigned char tem[8] = {0,0,0,0,0,0,0,0};

    //initialize the state to 0
    for (j = 0; j <= 37+8; j++) state[j] = 0;
    ...
    for (j = 0;  j < 16;  j = j+8)
    {
        acorn128_64steps(state, &(key[j]), tem, 0xff, 0xff, 1);
    }

    //load the iv
    for (j = 16;  j < 32;  j = j+8)
    {
        acorn128_64steps(state, &(iv[j-16]), tem, 0xff, 0xff, 1);
    }
    ...
    for (j = 40;  j < 224; j=j+8)
    {
        acorn128_64steps(state, &(key[j&0xf]), tem, 0xff, 0xff, 1);
    }
}
...
{
    unsigned char i;
    unsigned char plaintextbyte[8]  = {0,0,0,0,0,0,0,0};
    unsigned char ciphertextbyte[8] = {0,0,0,0,0,0,0,0};
    unsigned char ksbyte = 0;

    for (i = 0; i < (768-128)/8; i = i+8)
    {
        acorn128_64steps(state, plaintextbyte, ciphertextbyte, 0xff, 0xff,1);
    }
...
void acorn128_fixed_padding_256(unsigned char *state, unsigned char cb)
{
    ...
    for (i = 8; i < 128/8; i=i+8)
    {
        acorn128_64steps(state, plaintextbyte, ciphertextbyte, ca, cb, 1);
    }

    ca = 0;
    for (i = 0; i < 128/8; i=i+8)
    {
        acorn128_64steps(state, plaintextbyte, ciphertextbyte, ca, cb, 1);
    }
}
	...
	)
{
    ...
    unsigned char ciphertextbyte[8];
    unsigned char state[38+8];
    unsigned char ca, cb;
    ...
    ca = 0xff;
    cb = 0xff;
    for (i = 0; i < (adlen & 0xfffffffffffffff8ULL); i = i+8)
    {
        acorn128_64steps(state, &(ad[i]), ciphertextbyte, ca, cb, 1);
    }

    for (i = adlen & 0xfffffffffffffff8ULL; i < adlen; i=i+1)
    {
    ...
    {
        acorn128_64steps(state, &(m[i]), &c[i], ca, cb, 1);
    }
