...
/**********************************************************************
 * 
 * @name	:	mult_by_alpha_plus_one
 * 

 * @note	:	Multiplies given field element in "src" with \alpha+1, and
 * 				stores the result in "dest".

 * 
 **********************************************************************/	
void mult_by_alpha_plus_one(u8 *dest, u8 *src)
{
	u64 b,c;
	b =  load64(src);
	c =  load64(src);
...
void Proc_TXT(u8 *State, u8 *Delta,  u8 *output, u64 *outputlen, const u8 *input, u64 inputlen,  const u8 (*round_keys)[32], const u32 direction)
{
	if(inputlen != 0)
	{
		...
		if(inputlen < 16)
		{
			mult_by_alpha_plus_one(Delta, Delta); mult_by_alpha_plus_one(Delta, Delta); mult_by_alpha_plus_one(Delta, Delta); 	 
		}
		else
		{
			mult_by_alpha_plus_one(Delta, Delta); mult_by_alpha_plus_one(Delta, Delta);
		}
		gift_enc(State, &round_keys[0], State);			
		if(direction==0)
