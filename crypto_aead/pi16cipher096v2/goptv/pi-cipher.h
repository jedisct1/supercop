...
#define PROCESS_AD_BLOCK_NAME_Q(x)      pi ## x ## _process_ad_block_q
...
#define ENCRYPT_BLOCK_NAME_Q(x)         pi ## x ## _encrypt_block_q
...
#define PI_PROCESS_AD_BLOCK_Q           NAME(PROCESS_AD_BLOCK_NAME_Q, PI_WORD_SIZE)
...
#define PI_ENCRYPT_BLOCK_Q              NAME(ENCRYPT_BLOCK_NAME_Q, PI_WORD_SIZE)
...
typedef word_t qword_t __attribute__((vector_size(4 * sizeof(word_t))));
typedef word_t vchunk_t __attribute__((vector_size(4 * sizeof(word_t))));
typedef word_t chunk_t[4];
typedef qword_t qchunk_t[4];
typedef chunk_t state_t[4];
typedef vchunk_t vstate_t[4];
typedef qchunk_t qstate_t[4];
