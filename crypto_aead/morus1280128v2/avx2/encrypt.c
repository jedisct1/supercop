...
void morus_initialization(const unsigned char *key, const unsigned char *iv, __m256i *state)
{
	...
	memset(t, 0, 32);
	memcpy(t, key, 16);  memcpy(t + 16, key, 16);
...
void morus_tag_generation(unsigned long long msglen, unsigned long long adlen, unsigned char *c, __m256i *state)
{
	...
	tmp = XOR256(state[0], ROTL256_192(state[1]));
	tmp = XOR256(tmp, AND256(state[2], state[3]));

	_mm256_store_si256((__m256i*)t, tmp);
...
inline void morus_enc_aut_step(const unsigned char *plaintext, unsigned char *ciphertext, __m256i *state)
{
	__m256i keystream;
