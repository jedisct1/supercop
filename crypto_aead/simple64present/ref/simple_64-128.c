...
void derive_double(u8 *K_E, u8 *K_MAC, u8 *N2, u8 *N1, const u8 *k, const u8 *npub){
	...
	//(a) K_E
	memcpy(K_E, T1, MSZ);
	memcpy(&K_E[MSZ], T2, MSZ);
	
	//(b) K_MAC
	memcpy(K_MAC, T3, MSZ);
	memcpy(&K_MAC[MSZ], T4, MSZ);

	//(c) [N2, N1]
	memcpy(N1, T5, MSZ/2);
	memcpy(N2, &T5[MSZ/2], MSZ/2);
...
const unsigned char *k
	...
	*clen = mlen+CRYPTO_ABYTES;
	...
	/*** Derive keys ***/
	derive_double(K_E, K_MAC, N2, N1, k, npub);
