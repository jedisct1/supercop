...
static inline void aes128ni_setkey_e2d(const __m128i ekeys[11], __m128i dkeys[11]) {
  int i;
  dkeys[0] = ekeys[10];
#pragma unroll(9)
  for (i = 1 ; i < 10 ; i++) {
    dkeys[i] = _mm_aesimc_si128(ekeys[10-i]);
  }
  dkeys[10] = ekeys[0];
}

/** single, by-the-book AES encryption with AES-NI */
static inline __m128i aes128ni_encrypt1_si128(const __m128i nv, const __m128i rkeys[11]) {
  int i;
...
static inline void aes128ni_encrypt1(unsigned char *out, const unsigned char *n, __m128i rkeys[11]) {
  __m128i nv = _mm_loadu_si128((const __m128i *)n);
  __m128i temp = aes128ni_encrypt1_si128(nv, rkeys);
  _mm_storeu_si128((__m128i*)(out), temp);
}
static inline __m128i aes128ni_decrypt1_si128(const __m128i nv, const __m128i rkeys[11]) {
  int i;
  __m128i temp = _mm_xor_si128(nv, rkeys[0]);
#pragma unroll(9)
  for (i = 1 ; i < 10 ; i++) {
...
static inline void aes128ni_decrypt1(const unsigned char *n, unsigned char *out, const __m128i rkeys[11]) {
  __m128i nv = _mm_loadu_si128((const __m128i *)n);
  __m128i temp = aes128ni_decrypt1_si128(nv, rkeys);
  _mm_storeu_si128((__m128i*)(out), temp);
}
...
#define AESDECLASTx(a)                                  \
  temp##a = _mm_aesdeclast_si128(temp##a, rkeys[10])
...
#define FUNC(N, MAKEN, MAKENCOMMA)                          \
  static inline void aes128ni_encrypt##N(unsigned char *out, unsigned char *n, const __m128i rkeys[11]) { \
    MAKEN(NVx);                                                         \
    int i;                                                              \
    MAKEN(TEMPx);                                                       \
    for (i = 1 ; i < 10 ; i++) {                                        \
  ...
  static inline void aes128ni_decrypt##N(unsigned char *out, unsigned char *n, const __m128i rkeys[11]) { \
    MAKEN(NVx);                                                         \
    int i;                                                              \
    MAKEN(TEMPx);                                                       \
    for (i = 1 ; i < 10 ; i++) {                                        \
  ...
  static inline void aes128ni_encrypt##N##_xor_si128(unsigned char *out, \
							MAKENCOMMA(PARAMNV), \
							MAKENCOMMA(PARAMOFF), \
                                                     const __m128i rkeys[11]) { \
    int i;                                                              \
    MAKEN(TEMPx);                                                       \
    for (i = 1 ; i < 10 ; i++) {                                        \
      MAKEN(AESENCx);                                                   \
  ...
  static inline void aes128ni_decrypt##N##_xor_si128(unsigned char *out, \
                                                        MAKENCOMMA(PARAMNV), \
                                                        MAKENCOMMA(PARAMOFF), \
                                                     const __m128i rkeys[11]) { \
    int i;                                                              \
...
static void hash(block result, const unsigned char *k,
		 ...
		 const __m128i lstar,
		 const __m128i prelv[32], const __m128i aes_key[11]) {
	...
	tmpu3 = prelv[__builtin_ctz(i+3)];
        ...
        tmpu0 = aes128ni_encrypt1_si128(tmpu0, aes_key);
        tmpu1 = aes128ni_encrypt1_si128(tmpu1, aes_key);
        tmpu2 = aes128ni_encrypt1_si128(tmpu2, aes_key);
        tmpu3 = aes128ni_encrypt1_si128(tmpu3, aes_key);
	...
	tmp = prelv[__builtin_ctz(i)];
        ...
        tmp = aes128ni_encrypt1_si128(tmp, aes_key);
...
static int ocb_crypt(unsigned char *out, unsigned char *k, unsigned char *n,
    ...
    __m128i prelv[32];
    __m128i aes_decrypt_key[11];
    __m128i aes_encrypt_key[11];
    block ad_hash;
    ...
    aes128ni_setkey_encrypt(k, aes_encrypt_key);
    if ( ! encrypting ) {
	aes128ni_setkey_e2d(aes_encrypt_key, aes_decrypt_key);
    }
    ...
    lstar = aes128ni_encrypt1_si128(tmp, aes_encrypt_key);
    /* L_$ = double(L_*) */
    ...
    */
    ...
    ktop = aes128ni_encrypt1_si128(nonce, aes_encrypt_key);
    ...
    if (encrypting) {
      ...
      if (inbytes>=128) {
 	  ...
 	  inv0 = _mm_loadu_si128((const __m128i*)(in+0));
          ...
          aes128ni_encrypt8_xor_si128(out,tmpu0,tmpu1,tmpu2,tmpu3,
     ...
     if (inbytes>=64) {
	  ...
	  tmpu3 = prelv[__builtin_ctz(i+3)];
          ...
          aes128ni_encrypt4_xor_si128(out,tmpu0,tmpu1,tmpu2,tmpu3,
                                      offsetu0,offsetu1,offsetu2,offsetu3,
                                      aes_encrypt_key);
        }
      }
      ...
      for (; i<=inbytes/16; i++, in=in+16, out=out+16) {
        ...
        tmp = aes128ni_encrypt1_si128(tmp, aes_encrypt_key);
      ...
      if (inbytes>=128) {
        for (; i<=(inbytes/16)-7; i+=8, in+=128, out+=128) {
          ...
          aes128ni_decrypt8_xor_si128(out,tmpu0,tmpu1,tmpu2,tmpu3,
                                      tmpu4,tmpu5,tmpu6,tmpu7,
                                      offsetu0,offsetu1,offsetu2,offsetu3,
                                      offsetu4,offsetu5,offsetu6,offsetu7,
                                      aes_decrypt_key);
      ...
      if (inbytes>=64) {
        for (; i<=(inbytes/16)-3; i+=4, in+=64, out+=64) {
          ...
          aes128ni_decrypt4_xor_si128(out,tmpu0,tmpu1,tmpu2,tmpu3,
                                      offsetu0,offsetu1,offsetu2,offsetu3,
                                      aes_decrypt_key);
          sum = _mm_xor_si128(_mm_loadu_si128((const __m128i*)(out+0)), sum);
          sum = _mm_xor_si128(_mm_loadu_si128((const __m128i*)(out+16)), sum);
      ...
      for (; i<=inbytes/16; i++, in=in+16, out=out+16) {
        ...
        tmp = aes128ni_decrypt1_si128(tmp, aes_decrypt_key);
        outv = _mm_xor_si128(offset, tmp);
        _mm_storeu_si128((__m128i*)out, outv);
        /* Checksum_i = Checksum_{i-1} xor P_i */
    ...
    if (inbytes > 0) {
        ...
        pad = aes128ni_encrypt1_si128(offset, aes_encrypt_key);
    ...
    tag = aes128ni_encrypt1_si128(tmp, aes_encrypt_key);
