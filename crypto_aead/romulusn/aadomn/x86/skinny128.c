/******************************************************************************
 * Intel SSSE3 Skinny-128-384+ implementation fine tuned for Romulus-N/M (lower
...
#define SBOX_ARK_EVEN(rtk_23)                                                   \
    ...
    rtk   = _mm_xor_si128(rtk, rtk_1);      /* rtk_123 = rtk_23 ^ rtk_1 */      \
    tmp0  = _mm_srli_epi16(state, 4);       /* extract high nibbles (1/2) */    \
    ...
    rtk_1 = _mm_shuffle_epi8(rtk_1, perm_tk); /* perm for next rtk1 */          \
 ...
 * Apply the S-box, Add Round Tweakey, and Add Round Constants to the internal
 * state 'state'.
 * No TK1 addition is performed since its lower half is supposed to be null.
 */ 
#define SBOX_ARK_ODD(rtk_23)                                                    \
    tmp0  = _mm_srli_epi16(state, 4);       /* extract high nibbles (1/2) */    \
    state = _mm_and_si128(state, mask_nib); /* extract low nibbles */           \
    tmp0  = _mm_and_si128(tmp0, mask_nib);  /* extract high nibbles (2/2) */    \
    state = _mm_shuffle_epi8(s1, state);    /* apply inner S-box S1 */          \
 ...
 */
#define DOUBLE_ROUND(rtk_23)    \
    SBOX_ARK_EVEN(rtk_23);      \
    SR_MC();                    \
    SBOX_ARK_ODD(rtk_23+8);     \
    SR_MC();                    \
    ...
    const unsigned char *rtk_23)
{
    ...
    __m128i rtk_1 = _mm_loadu_si128((const __m128i*)tk1);
    ...
    __m128i perm_tk  = {0x0304060205000701, 0x0f0e0d0c0b0a0908};

    // skinny-128-384+ has 40 rounds
    DOUBLE_ROUND(rtk_23);
    DOUBLE_ROUND(rtk_23+16);
    DOUBLE_ROUND(rtk_23+32);
    DOUBLE_ROUND(rtk_23+48);
    DOUBLE_ROUND(rtk_23+64);
    DOUBLE_ROUND(rtk_23+80);
    DOUBLE_ROUND(rtk_23+96);
    DOUBLE_ROUND(rtk_23+112);
    DOUBLE_ROUND(rtk_23+128);
    DOUBLE_ROUND(rtk_23+144);
    DOUBLE_ROUND(rtk_23+160);
    DOUBLE_ROUND(rtk_23+176);
    DOUBLE_ROUND(rtk_23+192);
    DOUBLE_ROUND(rtk_23+208);
    DOUBLE_ROUND(rtk_23+224);
    DOUBLE_ROUND(rtk_23+240);
    DOUBLE_ROUND(rtk_23+256);
    DOUBLE_ROUND(rtk_23+272);
    DOUBLE_ROUND(rtk_23+288);
    DOUBLE_ROUND(rtk_23+304);
