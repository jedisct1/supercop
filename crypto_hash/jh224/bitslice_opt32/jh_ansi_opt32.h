/* This program gives the 32-bit optimized bitslice implementation of JH using ANSI C
   ...
   1) 83.0  cycles/byte   compiler: Intel C++ Compiler 11.1   compilation option: icc -O2
   2) 166   cycles/byte   compiler: gcc 4.3.2                 compilation option: gcc -O3  

   Microprocessor: Intel CORE 2 processor (Core 2 Quad Q6600 2.4GHz) 
   Operating System: 32-bit Windows Vista Business
   Speed for long message:
   3) 86.7  cycles/byte   compiler: Intel C++ Compiler 11.1    compilation option: icl /O2
   4) 153   cycles/byte   compiler: Microsoft Visual C++ 2008  compilation option: release
...
void E8(hashState *state) 
{
      ...
      /*perform 35.5 rounds*/
      for (roundnumber = 0; roundnumber < 35; roundnumber = roundnumber+7) {
      ...
      /*The last half round*/
      for (i = 0; i < 4; i++) {
            Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((uint32*)E8_bitslice_roundconstant[35])[i] );
            Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((uint32*)E8_bitslice_roundconstant[35])[i+4] );
      }          
