/* This program gives the 32-bit bitslice reference implementation of JH using ANSI C 
   ...
   1) 101 cycles/byte   compiler: Intel C++ compiler 11.1   compilation option: icc -O2
   2) 173 cycles/byte   compiler: gcc 4.3.2                 compilation option: gcc -O3  

   Microprocessor: Intel CORE 2 processor (Core 2 Quad Q6600 2.4GHz) 
   Operating System: 32-bit Windows Vista Business
   Speed for long message:
   3) 105 cycles/byte  compiler: Intel C++ compiler 11.1    compilation option: icl /O2
   4) 172 cycles/byte  compiler: Microsoft Visual C++ 2008  compilation option: release
...
void LastHalfRound(hashState* state, uint32 roundnumber);  /*the following defines the last half round of E8 */
...
void LastHalfRound(hashState* state, uint32 roundnumber) {                        
      uint32 i,temp0;
...
void E8(hashState *state)
{
      ...
      /*perform 35 rounds*/
      for (i = 0; i < 35; i++) RoundFunction(state, i);

      /*perform the last half round*/
      LastHalfRound(state, 35);                        
}
...
HashReturn Final(hashState *state, BitSequence *hashval) 
{
      if ( (state->databitlen & 0x1ff) == 0 ) {
            /*pad the message when databitlen is multiple of 512 bits, then hash the padded block*/
            memset(state->buffer, 0, 64);
            state->buffer[0]  = 0x80;
