/*This program gives the 64-bit optimized bitslice implementation of JH using ANSI C
   ...
   1) 37.3 cycles/byte  compiler: icc 11.1   compilation option: icc -O2
   2) 68.7 cycles/byte  compiler: gcc 4.3.2  compilation option: gcc -O3

   Microprocessor: Intel CORE 2 processor (Core 2 Quad Q6600 2.4GHz) 
   Operating System: 64-bit Windows Vista Business
   Speed for long message:
   3) 39.2 cycles/byte  compiler: Intel C++ compiler 11.1    compilation option: icl /O2
   4) 60.7 cycles/byte  compiler: Microsoft Visual C++ 2008  compilation option: release
...
void E8(hashState *state) 
{
      ...
      /*The last half round*/
      for (i = 0; i < 2; i++) {
            SS(state->x[0][i],state->x[2][i],state->x[4][i],state->x[6][i],state->x[1][i],state->x[3][i],state->x[5][i],state->x[7][i],((uint64*)E8_bitslice_roundconstant[35])[i],((uint64*)E8_bitslice_roundconstant[35])[i+2] );                  
      }          
