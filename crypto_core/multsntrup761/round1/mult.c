...
{
  const small *g = (const void *) kbytes;
  __m256i fgvec[96];
  __m256i fvec[48];
  __m256i gvec[24];
  Fq h[761];
  ...
  crypto_decode_pxint16(fvec,inbytes);

  for (i = 0;i < 761;++i)
    i[(int16 *) fvec] = Fq_freeze(i[(int16 *) fvec]);
  for (i = 0;i < 761;++i) {
    small gi = g[i];
  ...
  mult768_mix2_m256i(fgvec,fvec,gvec);
  fg = (Fq *) fgvec;

  h[0] = Fq_freeze(fg[0] + fg[p]);
  for (i = 1;i < 9;++i)
    h[i] = Fq_freeze(fg[i] + fg[i + p - 1] + fg[i + p]);
  for (i = 9;i < 761;i += 16) {
