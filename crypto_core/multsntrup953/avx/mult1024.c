...
static inline int16x16 squeeze_6343_x16(int16x16 x)
{
  return sub_x16(x,mullo_x16(mulhrs_x16(x,const_x16(5)),const_x16(6343)));
}
...
static inline int16x16 mulmod_6343_x16(int16x16 x,int16x16 y)
{
  int16x16 yqinv = mullo_x16(y,const_x16(10487)); /* XXX: precompute */
  int16x16 b = mulhi_x16(x,y);
  int16x16 d = mullo_x16(x,yqinv);
  int16x16 e = mulhi_x16(d,const_x16(6343));
  return sub_x16(b,e);
}
...
#define hpad fpad
  ...
  for (i = 0;i < 2048;i += 16) {
    ...
    t = add_x16(u1,mulmod_6343_x16(t,const_x16(1308)));
    store_x16(&h[i],t);
  }
}
...
static inline int16x16 freeze_6343_x16(int16x16 x)
{
  int16x16 mask, xq;
...
#define h f
  ...
  for (i = 0;i < 1024;i += 16) {
    x = load_x16(&f[i]);
    x = freeze_6343_x16(squeeze_6343_x16(x));
    store_x16(&f[i],x);
  }
  for (i = 0;i < p;++i) {
    int8 gi = kbytes[i];
