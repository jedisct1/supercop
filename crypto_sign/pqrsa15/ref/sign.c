...
#if crypto_sign_PUBLICKEYBYTES != K * PBYTES
#error "PUBLICKEYBYTES must be K * PBYTES"
#endif
#if crypto_sign_SECRETKEYBYTES != 3 * K * PBYTES
#error "SECRETKEYBYTES must be 3 * K * PBYTES"
#endif
#if crypto_sign_BYTES != 32 + K * PBYTES
#error "BYTES must be 32 + K * PBYTES"
#endif
...
)
{
  ...
  memmove(sm + MAXNBYTES + 32,m,mlen);
  *smlen = MAXNBYTES + 32 + mlen;
  randombytes(sm + MAXNBYTES,32);

  nbytes = mpz_sizeinbase(N,256);
  KeccakWidth1600_Sponge(1088,512,sm + MAXNBYTES,32 + mlen,0x1F,ystr,nbytes - 1);

  gmp_import(X,ystr,nbytes - 1);
  cuberoot(X,p,pinv,N);
  gmp_export(sm,MAXNBYTES,X);
