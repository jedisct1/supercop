...
#define OverWr( argIndex )  STORE256u(stateAsLanes[argIndex], LOAD4_64(curData3[argIndex], curData2[argIndex], curData1[argIndex], curData0[argIndex]))
...
#define OverWr4( argIndex )     lanes0 = LOAD256u( curData0[argIndex]),\
        lanes1 = LOAD256u( curData1[argIndex]),\
                 lanes2 = LOAD256u( curData2[argIndex]),\
                          lanes3 = LOAD256u( curData3[argIndex]),\
                                   INTLEAVE(),\
                                   STORE256u( stateAsLanes[argIndex+0], lanes0 ),\
                                   STORE256u( stateAsLanes[argIndex+1], lanes1 ),\
                                   STORE256u( stateAsLanes[argIndex+2], lanes2 ),\
                                   STORE256u( stateAsLanes[argIndex+3], lanes3 )
...
#define Extr4( argIndex )   lanes0 = LOAD256u( stateAsLanes[argIndex+0] ),           \
                                     lanes1 = LOAD256u( stateAsLanes[argIndex+1] ),           \
                                             lanes2 = LOAD256u( stateAsLanes[argIndex+2] ),           \
                                                     lanes3 = LOAD256u( stateAsLanes[argIndex+3] ),           \
                                                             UNINTLEAVE(),                                           \
...
#define ExtrXor4( argIndex ) \
    lanes0 = LOAD256u( stateAsLanes[argIndex+0] ),\
             lanes1 = LOAD256u( stateAsLanes[argIndex+1] ),\
                      lanes2 = LOAD256u( stateAsLanes[argIndex+2] ),\
                               lanes3 = LOAD256u( stateAsLanes[argIndex+3] ),\
                                        UNINTLEAVE(),\
...
#define copyFromState(X, state) \
    X##ba = LOAD256u(state[ 0]); \
    X##be = LOAD256u(state[ 1]); \
    X##bi = LOAD256u(state[ 2]); \
    X##bo = LOAD256u(state[ 3]); \
    X##bu = LOAD256u(state[ 4]); \
    X##ga = LOAD256u(state[ 5]); \
    X##ge = LOAD256u(state[ 6]); \
    X##gi = LOAD256u(state[ 7]); \
    X##go = LOAD256u(state[ 8]); \
    X##gu = LOAD256u(state[ 9]); \
    X##ka = LOAD256u(state[10]); \
    X##ke = LOAD256u(state[11]); \
    X##ki = LOAD256u(state[12]); \
    X##ko = LOAD256u(state[13]); \
    X##ku = LOAD256u(state[14]); \
    X##ma = LOAD256u(state[15]); \
    X##me = LOAD256u(state[16]); \
    X##mi = LOAD256u(state[17]); \
    X##mo = LOAD256u(state[18]); \
    X##mu = LOAD256u(state[19]); \
    X##sa = LOAD256u(state[20]); \
    X##se = LOAD256u(state[21]); \
    X##si = LOAD256u(state[22]); \
    X##so = LOAD256u(state[23]); \
    X##su = LOAD256u(state[24]); \

#define copyToState(state, X) \
    STORE256u(state[ 0], X##ba); \
    STORE256u(state[ 1], X##be); \
    STORE256u(state[ 2], X##bi); \
    STORE256u(state[ 3], X##bo); \
    STORE256u(state[ 4], X##bu); \
    STORE256u(state[ 5], X##ga); \
    STORE256u(state[ 6], X##ge); \
    STORE256u(state[ 7], X##gi); \
    STORE256u(state[ 8], X##go); \
    STORE256u(state[ 9], X##gu); \
    STORE256u(state[10], X##ka); \
    STORE256u(state[11], X##ke); \
    STORE256u(state[12], X##ki); \
    STORE256u(state[13], X##ko); \
    STORE256u(state[14], X##ku); \
    STORE256u(state[15], X##ma); \
    STORE256u(state[16], X##me); \
    STORE256u(state[17], X##mi); \
    STORE256u(state[18], X##mo); \
    STORE256u(state[19], X##mu); \
    STORE256u(state[20], X##sa); \
    STORE256u(state[21], X##se); \
    STORE256u(state[22], X##si); \
    STORE256u(state[23], X##so); \
    STORE256u(state[24], X##su); \
