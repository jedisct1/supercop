...
void GF_mul(GF c, const GF a, const GF b)
{
  ...
  t[0] = temp[2] ^ ((temp[3] >> 57) ^ (temp[3] >> 62) ^ crypto_uint64_topbit_01(temp[3]));

  c[1] = temp[1] ^ temp[3];
  c[1] ^= (temp[3] << 7) | (t[0] >> 57);
  c[1] ^= (temp[3] << 2) | (t[0] >> 62);
  c[1] ^= (temp[3] << 1) | crypto_uint64_topbit_01(t[0]);
...
void GF_transposed_matmul(GF c, const GF a, const GF b[AIM2_NUM_BITS_FIELD])
{
  ...
  for (size_t i = AIM2_NUM_WORDS_FIELD; i; --i, ++a_ptr)
  {
    ...
    for (size_t j = AIM2_NUM_BITS_WORD; j; j -= 8, index >>= 8, b_ptr += 8)
    {
      ...
      mask = -(crypto_int64_bitmod_01(index,4));
      temp_c0 ^= (b_ptr[4][0] & mask);
      temp_c1 ^= (b_ptr[4][1] & mask);

      mask = -(crypto_int64_bitmod_01(index,5));
      temp_c0 ^= (b_ptr[5][0] & mask);
      temp_c1 ^= (b_ptr[5][1] & mask);

      mask = -(crypto_int64_bitmod_01(index,6));
      temp_c0 ^= (b_ptr[6][0] & mask);
      temp_c1 ^= (b_ptr[6][1] & mask);

      mask = -(crypto_int64_bitmod_01(index,7));
...
void GF_sqr_s(GF c, const GF a)
{
  ...
  t = temp[2] ^ ((temp[3] >> 57) ^ (temp[3] >> 62) ^ crypto_uint64_topbit_01(temp[3]));

  c[1] = temp[1] ^ temp[3];
  c[1] ^= (temp[3] << 7) | (t >> 57);
  c[1] ^= (temp[3] << 2) | (t >> 62);
  c[1] ^= (temp[3] << 1) | crypto_uint64_topbit_01(t);
