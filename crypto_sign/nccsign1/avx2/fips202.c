...
/*
static void state_to_avx_struct(uint64_t *s, KeccakP1600_AVX2_state *state)
{
  state->Aba = s[ 0];
  state->Abe = s[ 1];
  state->Abi = s[ 2];
...
static void avx_struct_to_state(KeccakP1600_AVX2_state *state, uint64_t *s)
{
  s[ 0] = state->Aba;
  s[ 1] = state->Abe;
  s[ 2] = state->Abi;
                                  ...
                                  size_t inlen)
{
  ...
  while(pos+inlen >= r) {
    ...
    // state_to_avx_struct(s, &state);
    KeccakF1600_StatePermute(s);
    // KeccakP1600_Permute_24rounds(&state);
    // avx_struct_to_state(&state, s);
