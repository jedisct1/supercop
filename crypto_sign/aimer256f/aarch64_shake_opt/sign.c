...
void commit_and_expand_tape_x2(unsigned char *commits, tape_t *tapes,
                               const hash_instance_x2 *ctx_precom,
                               const unsigned char *seeds,
                               unsigned long long rep, unsigned long long party)
{
  hash_instance_x2 ctx;
  unsigned char bufs[2][AIMER_SEED_SIZE + 2];
  const unsigned char* in_ptrs[2] = {bufs[0], bufs[1]};
  unsigned char* out_ptrs[2];

  hash_ctx_clone_x2(&ctx, ctx_precom);
  ...
  hash_squeeze_x2(&ctx, out_ptrs, AIMER_COMMIT_SIZE);

  out_ptrs[0] = (unsigned char *)(tapes);
  out_ptrs[1] = (unsigned char *)(tapes + 1);

  hash_squeeze_x2(&ctx, out_ptrs, sizeof(tape_t));
}
                 ...
                 const unsigned char *sk, const unsigned char *m, unsigned long long mlen)
{
  ...
  hash_instance_x2 ctx_precom;
  hash_init_prefix_x2(&ctx_precom, HASH_PREFIX_5);
  hash_update_x2_1(&ctx_precom, sign->salt, AIMER_SALT_SIZE);
  ...
  for (unsigned long long rep = 0; rep < AIMER_T; rep++)
  {
    ...
    for (unsigned long long party = 0; party < AIMER_N; party += 2)
    {
      // generate execution views and commitments
      commit_and_expand_tape_x2(commits[rep][party], tapes, &ctx_precom,
                                nodes[rep][party + AIMER_N - 1], rep, party);
      hash_update(&ctx, commits[rep][party], 2 * AIMER_COMMIT_SIZE);
        ...
        const unsigned char *pk)
{
  ...
  for (unsigned long long rep = 0; rep < AIMER_T; rep++)
  {
    ...
    {
      tape_t tapes[2];
      unsigned char commits[2][AIMER_COMMIT_SIZE];
      commit_and_expand_tape_x2(commits[0], tapes, &ctx_precom,
                                nodes[party + AIMER_N - 2], rep, party);

      if (party / 2 == i_bar / 2)
      {
        memcpy(commits[i_bar % 2], sign->proofs[rep].missing_commitment,
               AIMER_COMMIT_SIZE);
      }
      hash_update(&ctx_h1, commits[0], 2 * AIMER_COMMIT_SIZE);
      ...
      if (party == AIMER_N - 2)
      {
        ...
        GF_add(tapes[1].pt_share, tapes[1].pt_share, temp);

        GF_from_bytes(temp, sign->proofs[rep].delta_ts_bytes[0]);
        GF_add(tapes[1].t_shares[0], tapes[1].t_shares[0], temp);
        GF_from_bytes(temp, sign->proofs[rep].delta_ts_bytes[1]);
        GF_add(tapes[1].t_shares[1], tapes[1].t_shares[1], temp);
        GF_from_bytes(temp, sign->proofs[rep].delta_ts_bytes[2]);
        GF_add(tapes[1].t_shares[2], tapes[1].t_shares[2], temp);

        GF_from_bytes(temp, sign->proofs[rep].delta_c_bytes);
        GF_add(tapes[1].c_share, tapes[1].c_share, temp);
      }
