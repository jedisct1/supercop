...
unsigned int rej_uniform(int32_t *a, unsigned int len, const uint8_t *buf,
    ...
    while (ctr < len && pos + 2 <= buflen) {
        ...
        tlessQ = crypto_uint32_smaller_mask(t,Q);
...
#if ETA == 1
        uint32_t t = buf[pos++];
        uint32_t tless243 = crypto_uint32_smaller_mask(t,243);
        crypto_declassify(&tless243,sizeof tless243);
        if (tless243) {
            // reduce mod 3
            a[ctr++] = mod3(t);
...
#elif ETA == 2
        uint32_t t0, t1;
        crypto_uint32 t0less15, t1less15;
        t0 = buf[pos] & 0x0F;
        t1 = buf[pos++] >> 4;
        t0less15 = crypto_uint32_smaller_mask(t0,15);
        crypto_declassify(&t0less15,sizeof t0less15);
        if (t0less15) {
            t0 = t0 - (205 * t0 >> 10) * 5;
            a[ctr++] = 2 - t0;
        }
        t1less15 = crypto_uint32_smaller_mask(t1,15);
        crypto_declassify(&t1less15,sizeof t1less15);
        if (t1less15 && ctr < len) {
            t1 = t1 - (205 * t1 >> 10) * 5;
            a[ctr++] = 2 - t1;
        }
...
int sample_gauss(uint64_t *r, fp96_76 *sqsum, const uint8_t *buf, const size_t buflen, const size_t len, const int dont_write_last)
{
    ...
    while (coefcnt < len) {
        ...
        crypto_declassify(&accepted,sizeof accepted);
        cnt += 1;
        coefcnt += accepted;
