                    ...
                    size_t data_len)
{
  ...
  while (i < data_len)
  {
    if ((ctx->byte_io_index == 0) && (data_len >= (i + SHAKE256_RATE)))
    {
      ...
      {
        add_bytes_x2(ctx->state_x2, data, data_index, 0, SHAKE256_RATE);
        data_index += SHAKE256_RATE;
...
void hash_final_x2(hash_instance_x2 *ctx)
{
  ...
  lane_position  = (SHAKE256_RATE - 1) >> 3;
  offset_in_lane = (SHAKE256_RATE - 1) & 0x07;
  ...
  while (i < buffer_len)
  {
        ...
        && (buffer_len >= (i + SHAKE256_RATE)))
    {
      for (j = buffer_len - i; j >= SHAKE256_RATE; j -= SHAKE256_RATE)
      {
        ...
        for (k = 0; k < SHAKE256_RATE / 8 - 1; k += 4)
        {
