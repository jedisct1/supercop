...
{
  shake256_init(ctx);
  shake256_absorb(ctx, &prefix, sizeof(prefix));
}

void hash_update(hash_instance *ctx, const uint8_t *data, size_t data_len)
{
  shake256_absorb(ctx, data, data_len);
}
...
{
  shake256_squeeze(buffer, buffer_len, ctx);
}
                    ...
                    size_t data_len)
{
  ...
  while (i < data_len)
  {
    if ((ctx->byte_io_index == 0) && (data_len >= (i + SHAKE256_RATE)))
    {
      for (j = data_len - i; j >= SHAKE256_RATE; j -= SHAKE256_RATE)
      {
        for (size_t index = 0; index < 4; index++)
        {
          KeccakP1600times4_AddBytes(&ctx->states, index, curData[index], 0,
                                     SHAKE256_RATE);
          curData[index] += SHAKE256_RATE;
        }
    ...
    else
    {
      ...
      if (partial_block + ctx->byte_io_index > SHAKE256_RATE)
      {
        partial_block = SHAKE256_RATE - ctx->byte_io_index;
      }
      i += partial_block;
      ...
      ctx->byte_io_index += partial_block;
      if (ctx->byte_io_index == SHAKE256_RATE)
      {
        KeccakP1600times4_PermuteAll_24rounds(&ctx->states);
        ctx->byte_io_index = 0;
      }
...
void hash_final_x4(hash_instance_x4 *ctx)
{
  ...
  for (size_t index = 0; index < 4; index++)
  {
    KeccakP1600times4_AddByte(&ctx->states, index, 0x80, SHAKE256_RATE - 1);
  }
...
void hash_squeeze_x4(hash_instance_x4 *ctx, uint8_t **buffer, size_t buffer_len)
{
  ...
  {
    if ((ctx->byte_io_index == SHAKE256_RATE) &&
        (buffer_len >= (i + SHAKE256_RATE)))
    {
      for (j = buffer_len - i; j >= SHAKE256_RATE; j -= SHAKE256_RATE)
      {
        KeccakP1600times4_PermuteAll_24rounds(&ctx->states);
        for(size_t index = 0; index < 4; index++)
        {
