...
int mqlr_sign(uint8_t* signature, const sk_mqlr* sk, const uint8_t* m, const uint32_t mlen)
{
	...
	uint8_t _ALIGN_(32) mat[_O * _O_BYTE];
	uint8_t _ALIGN_(32) mat_tmp[_O * _O_BYTE];

	uint8_t m_salt[mlen + _SALT_BYTE];
	uint8_t* salt = m_salt + mlen;
	...
	// roll vinegars.
	uint8_t _ALIGN_(32) vinegar[_V_BYTE];
	...
	uint8_t _ALIGN_(32) r_l1_F1[_O_BYTE] = { 0 };
	uint8_t _ALIGN_(32) multab[_V * 2 * 16];
	uint8_t temp_o[_MAX_O_BYTE + 32] = { 0 };

	memcpy(m_salt, m, mlen);									// input: M||salt

	hash_msg(y, _PUB_M_BYTE, m_salt, mlen + _SALT_BYTE);
...
rej: ;
	...
	gfv_generate_multab(multab, vinegar, _V);

	gf256mat_prod_multab_avx2(mat_tmp, sk->Fq2, _O * _O, _V, multab);

	circmat_eval_multab(r_l1_F1, sk->Fq1, multab, _V, _O);
	memcpy(temp_o, r_l1_F1, _O_BYTE);

	rr = gfmat_solve_linear_eq(mat_tmp, _O_BYTE, B, C, D, A_inv);
...
// remain part of linear solve
	...
	gf256mat_prod_avx2(temp_vec + H_half, CA_inv, H_half, H_half, temp_o);
	gf256v_set_zero(temp_vec, H_half);

	gf256v_add_avx2(temp_vec, temp_o, _O_BYTE);
	...
	gf256mat_prod_avx2(x_o, A_inv, H_half, H_half, temp_vec);

	gf256v_set_zero(x_o + H_half, H_half);
	gf256v_add_avx2(x_o, temp_vec2, _O_BYTE);
// remain part of linear solve
...
int mqlr_verify(const uint8_t* m, const uint32_t mlen, const uint8_t* signature, const uint8_t* pk)
{
	...
	mqlr_pubmap(digest_ck, pk, signature);

	unsigned char correct[_PUB_M_BYTE];
	unsigned char m_salt[mlen + _SALT_BYTE];
	memcpy(m_salt, m, mlen);
	memcpy(m_salt + mlen, signature + _PUB_N_BYTE, _SALT_BYTE);
	hash_msg(correct, _PUB_M_BYTE, m_salt, mlen + _SALT_BYTE);
