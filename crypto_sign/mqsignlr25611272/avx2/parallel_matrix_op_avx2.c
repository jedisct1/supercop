	...
	const unsigned char* B, unsigned Bheight, unsigned size_Bcolvec, unsigned Bwidth, unsigned size_batch)
{
	...
	for (unsigned i = 0; i < Aheight; i++) {
		for (unsigned j = 0; j < Bwidth; j++) {
			for (unsigned k = 0; k < Bheight; k++) {
				gf256v_madd_multab_avx2(r, &bA[size_batch * (i + k * Aheight)], &B[(j * Bheight + k) * 32], align_len);
			}
			memcpy(bC, r, size_batch);
			memset(r, 0, size_batch);
			bC += size_batch;
	...
	const unsigned char* B, unsigned Bheight, unsigned size_Bcolvec, unsigned Bwidth, unsigned size_batch)
{
	...
	for (unsigned i = 0; i < Aheight; i++) {
		for (unsigned j = 0; j < Bwidth; j++) {
			for (unsigned k = 0; k < Bheight; k++) {
				gf256v_madd_multab_avx2(r, &bA[k * size_batch], &B[(j * Bheight + k) * 32], align_len);
			}
			memcpy(bC, r, size_batch);
			memset(r, 0, size_batch);
			bC += size_batch;
...
void sparmat_vv_eval_multab_avx2(unsigned char* y, const unsigned char* sparmat, const unsigned char* x_multab, unsigned size_batch)
{
	...
	unsigned char align_len = ((size_batch + 31) / 32) * 32;
	unsigned char _z[_O << 1] = { 0, };

	memset(y, 0, _O);
	for (unsigned i = 0; i < _V; i++) {
		memset(tmp, 0, size_batch);
		gf256v_madd_multab_avx2(tmp, sparmat + i * _O, x_multab + i * 32, align_len);
	...
	for (unsigned i = 0; i < _V; i++) {
		gf256v_madd_multab_avx2(_z, rot_mat + i * _O, x_multab + i * 32, align_len);
	}
	memcpy(y, _z, _O);
}
