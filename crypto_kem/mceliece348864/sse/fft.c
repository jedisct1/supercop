...
#include "consts.data"
	...
	for (i = 0; i < 32; i+=2)
	{
		...
		for (b = 0; b < GFBITS; b++) out[i+0][b] ^= tmp[b];
		for (b = 0; b < GFBITS; b++) out[i+1][b] ^= out[i+0][b];

		for (b = 0; b < GFBITS; b++) x[b] = vec128_unpack_low(out[i+0][b], out[i+1][b]);
		for (b = 0; b < GFBITS; b++) y[b] = vec128_unpack_high(out[i+0][b], out[i+1][b]);

		for (b = 0; b < GFBITS; b++) out[i+0][b] = x[b];
		for (b = 0; b < GFBITS; b++) out[i+1][b] = y[b];
	}
...
#include "powers.data"
	...
	for (i = 0; i < 32; i++)
		for (b = 0; b < GFBITS; b++) out[i][b] ^= powers[i][b];
}

void fft(vec128 out[][ GFBITS ], uint64_t *in)
{
