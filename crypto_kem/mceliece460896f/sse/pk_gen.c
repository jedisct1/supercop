...
static int mov_columns(uint64_t mat[][ nBlocks_I*2 ], int16_t * pi, uint64_t * pivots)
{
	int i, j, pivot_col[32];
	uint64_t buf[32], t, d, one = 1; 
...
int pk_gen(unsigned char * pk, const unsigned char * irr, uint32_t * perm, int16_t * pi, uint64_t * pivots)
{
	...
	for (j = 9; j < 36; j += par_width)
	{
		...
		if (j == 9)
		{
			...
			{
				k = 1;
				store_i(pk + PK_ROW_BYTES * row + j*16 + k*8 + 4 - 13*12, par.w[row][k] >> 32, 4);

				for (k = 2; k < 18; k++)
					store8(pk + PK_ROW_BYTES * row + j*16 + k*8 - 13*12, par.w[row][k]);
			}
		}
		else
		{
