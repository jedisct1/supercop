...
Alternative in timesx, divx: Use loads and stores.
See partial example in vec256_timesx_3.
*/
...
{
  int i;
  small srev[768+(768-p)];
  small si;
  small g0[768];
  small g1[768];

  reverse(srev,s);
  for (i = p;i < 768;++i) srev[i+768-p] = 0;
...
static void vec256_final(small *out,const vec256 *V0,const vec256 *V1)
{
  ...
  reverse(vrev,v);
  for (i = 768;i < 768+(768-p);++i) vrev[i] = 0;
...
/*
out = s^(-1) mod f, returning 0, if s is invertible mod f
or returning -1 if s is not invertible mod f
out,s are polys of degree <p
f is x^p-x-1; change F initialization to use another (reversed) poly
*/
int r3_recip(small *out,const small *s)
{
  vec256 F0[3];
  vec256 F1[3];
  vec256 G0[3];
  ...
  F1[1] = F0[1] = _mm256_set1_epi32(0);
  F1[2] = F0[2] = _mm256_set_epi32(0,0,0,0,1073741824,0,1073741824,0);
  ...
  for (loop = 512;loop < p + (p-1-512);++loop) {
    vec256_timesx_3(V0);
    vec256_timesx_3(V1);
    swapmask = negative_mask(minusdelta) & vec256_bit0mask(G0);
