...
void encapsulate(
    ...
    gf_t sk_expanded,b,c;
    limb_t *a=sk_expanded;
    for (unsigned i=0; i<DIM; i++) {
        noise(&c,ctx,i+DIM,1);
        for (unsigned j=0; j<DIM; j++) {
            uniform(&b,pk,j+DIM*i,1);
            noise(&sk_expanded,ctx,j,1);
            mac(c,b,sk_expanded);
        }
...
#error "buffer management: ENC_SEED_BYTES > SHARED_SECRET_BYTES"
    ...
    for (unsigned i=0; i<DIM; i++) {
        ...
        mac(d,b,c);
        noise(&b,ctx,i+DIM,1);
        for (unsigned j=0; j<NLIMBS; j++) c[j] += b[j];
        contract(a.buf, c);
    ...
    hash_output(ctx2,a.buf,PRF_KEY_BYTES);
    
    /* Calculate the PRF */
    threebears_hash_init(ctx,HASH_PURPOSE_PRF);
    hash_update(ctx,a.buf,PRF_KEY_BYTES);
    hash_update(ctx,capsule,CAPSULE_BYTES);
    hash_output(ctx,a.buf,SHARED_SECRET_BYTES);

    for (unsigned i=0; i<SHARED_SECRET_BYTES; i++) {
        shared_secret[i] = (shared_secret[i] & ok) | (a.buf[i] & ~ok);
    }
...
#if MATRIX_SEED_BYTES > SHARED_SECRET_BYTES
#error "buffer management: MATRIX_SEED_BYTES > SHARED_SECRET_BYTES"
#endif
    uint8_t *seed = shared_secret;
