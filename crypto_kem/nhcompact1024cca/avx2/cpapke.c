...
{
  poly_tobytes(r, pk);
  memcpy(r+NEWHOPECMPCT_POLYBYTES,seed,NEWHOPECMPCT_SYMBYTES);
}
...
{
  poly_frombytes(pk, r);
  memcpy(seed,r+NEWHOPECMPCT_POLYBYTES,NEWHOPECMPCT_SYMBYTES);
}
...
{
  poly_tobytes(r,b);
  poly_compress(r+NEWHOPECMPCT_POLYBYTES,v);
}
...
{
  poly_frombytes(b, r);
  poly_decompress(v, r+NEWHOPECMPCT_POLYBYTES);
}
                    ...
                    unsigned char *sk)
{
  ...
  unsigned char z[2*NEWHOPECMPCT_SYMBYTES];
  unsigned char *publicseed = z;
  unsigned char *noiseseed = z+NEWHOPECMPCT_SYMBYTES;

  randombytes(z, NEWHOPECMPCT_SYMBYTES);
  shake256(z, 2*NEWHOPECMPCT_SYMBYTES, z, NEWHOPECMPCT_SYMBYTES);
                ...
                const unsigned char *coin)
{
  ...
  unsigned char publicseed[NEWHOPECMPCT_SYMBYTES];
