...
// 20251220 djb: fork mlkem from kyber
...
static uint32_t load24_littleendian(const uint8_t x[3]) {
  uint32_t r = 0;
  int i;
  for (i = 0; i < 3; i++) r |= (uint32_t)x[i] << (8 * i);
  return r;
}
...
static void poly_getnoise_3(poly *r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce) {
...
static void indcpa_keypair(uint8_t pk[crypto_kem_PUBLICKEYBYTES], uint8_t sk[KYBER_POLYVECBYTES]) {
  ...
  gen_a(a, publicseed);
  for (i = 0; i < KYBER_K; i++) poly_getnoise_3(&skpv.vec[i], noiseseed, nonce++);
  for (i = 0; i < KYBER_K; i++) poly_getnoise_3(&e.vec[i], noiseseed, nonce++);
  polyvec_ntt(&skpv);
  polyvec_ntt(&e);
  for (i = 0; i < KYBER_K; i++) {
...
static void indcpa_enc(uint8_t c[crypto_kem_CIPHERTEXTBYTES], const uint8_t m[KYBER_SYMBYTES], const uint8_t pk[crypto_kem_PUBLICKEYBYTES], const uint8_t coins[KYBER_SYMBYTES]) {
  ...
  poly_frommsg(&k, m);
  gen_at(at, seed);
  for (i = 0; i < KYBER_K; i++) poly_getnoise_3(sp.vec + i, coins, nonce++);
...
int crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {
  ...
  uint8_t buf2[KYBER_SYMBYTES+crypto_kem_CIPHERTEXTBYTES];
  ...
  memcpy(buf2, sk + crypto_kem_SECRETKEYBYTES - KYBER_SYMBYTES, KYBER_SYMBYTES);
  memcpy(buf2 + KYBER_SYMBYTES, ct, crypto_kem_CIPHERTEXTBYTES);
  SHAKE256(ss, KYBER_SYMBYTES, buf2, KYBER_SYMBYTES + crypto_kem_CIPHERTEXTBYTES);
