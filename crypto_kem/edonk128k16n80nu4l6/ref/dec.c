...
{
    int i;
    unsigned char hash[crypto_hash_sha256_BYTES], hash2[crypto_hash_sha256_BYTES], temphash[2*crypto_hash_sha256_BYTES];
    ...
    for(i=0; i<ErrorBasisDimension; i+=2) {
        ...
        crypto_hash_sha256(hash2, temphash, 2*crypto_hash_sha256_BYTES);

        if(memcmp(hash2, HashedSecretShare, crypto_hash_sha256_BYTES) == 0) {
...
int crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk)
{
    ...
    Kfield_init();
//    struct PrivateKeyStruct PrivateKey;
