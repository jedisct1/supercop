...
_INLINE_ uint32_t sha_update(sha512_hash_t *hash, const uint8_t *msg, uint32_t n)
{
    ...
    while(n>0)
    {
        a = hash->qw[0]; b = hash->qw[1];
        c = hash->qw[2]; d = hash->qw[3];
        e = hash->qw[4]; f = hash->qw[5];
        g = hash->qw[6]; h = hash->qw[7];
        ...
        hash->qw[0] += a; hash->qw[1] += b;
        hash->qw[2] += c; hash->qw[3] += d;
        hash->qw[4] += e; hash->qw[5] += f;
        hash->qw[6] += g; hash->qw[7] += h;
        ...
        IN const uint8_t *msg)
{
    ...
    uint64_t tmp[SHA512_HASH_QWORDS] = INIT_HASH;
    sha512_hash_t hash;
	memcpy(hash.raw, (uint8_t*)tmp, sizeof(hash));
    ...
    {
        hash_out->qw[i] = bswap_64(hash.qw[i]);
    }
#endif 
