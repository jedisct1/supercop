                      ...
                      IN const sk_t *sk)
{
...
#elif BIKE_VER==2
    pad_ct_t pad_ct = {.val=*ct};
    gf2x_mod_mul(pad_s[0].qw, pad_ct.qw, pad_sk[0].qw);

#elif BIKE_VER==3
    // BIKE3 syndrome: s = c0 + c1*h0
    // NTL is better in this case.
    cyclic_product(pad_s[0].val.raw, ct->val[1].raw, pad_sk[0].val.raw);
    gf2x_add(pad_s[0].val.raw, pad_s[0].val.raw, ct->val[0].raw, R_SIZE);
#endif
    ...
    secure_clean(pad_s[0].raw, sizeof(pad_s));
    secure_clean(pad_sk[0].raw, sizeof(pad_sk));
#if (BIKE_VER != 3)
           ...
           IN const uint32_t u)
{
    ...
    for(ctx.delta = MAX_DELTA; 
       (ctx.delta >= 0) && (count_ones(s->dup1.raw, sizeof(s->dup1)) > u); 
        ctx.delta--)
    {
        // Reset the error
        memset(e, 0, sizeof(*e));
