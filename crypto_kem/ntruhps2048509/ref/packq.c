...
void poly_Sq_tobytes(unsigned char *r, const poly *a)
{
  ...
  for(i=0;i<NTRU_PACK_DEG/8;i++)
  {
    ...
    r[11 * i + 6] = (unsigned char) ((t[4] >>  4) | ((crypto_int64_bottombit_01(t[5])) << 7));
...
void poly_Sq_frombytes(poly *r, const unsigned char *a)
{
  ...
  for(i=0;i<NTRU_PACK_DEG/8;i++)
  {
    ...
    r->coeffs[8*i+2] = (a[11*i+ 2] >> 6) | (((uint16_t)a[11*i+ 3] & 0xff) << 2) | ((crypto_int64_bottombit_01((uint16_t)a[11*i+ 4])) << 10);
    r->coeffs[8*i+3] = (a[11*i+ 4] >> 1) | (((uint16_t)a[11*i+ 5] & 0x0f) << 7);
    r->coeffs[8*i+4] = (a[11*i+ 5] >> 4) | (((uint16_t)a[11*i+ 6] & 0x7f) << 4);
    r->coeffs[8*i+5] = crypto_uint8_topbit_01(a[11*i+ 6]) | (((uint16_t)a[11*i+ 7] & 0xff) << 1) | (((uint16_t)a[11*i+ 8] & 0x03) <<  9);
